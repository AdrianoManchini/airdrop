...
bot.onText(/\/start/, (msg) => {
    bot.sendPhoto(msg.chat.id,img_url,{caption : "Welcome to BITCOZ Airdrop! 😍😍 \nPlease join our community and get 100 token.\n \n "}).then(() => {
        var option = {
            "reply_markup": {
                "keyboard": [["1. Join the BITCOZ Telegram group", "2. Your Telegram Username"],   ["3. E-mail address" , "4. ETH address (No exchange wallet!)"]]
                }
        };
        bot.sendMessage(msg.chat.id,"Airdrop Rules ⚔️⚔️\n 1. Join the BITCOZ Telegram group \n 2. Your Telegram Username \n 3. E-mail address \n 4. ETH address (No exchange wallet!) \n",option);
    })
})

bot.on('message', (msg) => {
    var send_text = msg.text;
    var step1_text = '1. Join the BITCOZ Telegram group'
    if (send_text.toString().indexOf(step1_text) === 0) {
        var text = 'BITCOZ Telegram Group';
        var keyboardStr = JSON.stringify({
            inline_keyboard: [
            [
                {text:'Join the chat',url:'https://t.me/joinchat/IdaFMA9yh0RNxxTO6uo7Rw'}
            ]
            ]
        });
